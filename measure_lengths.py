...
def main(glacier: str = "vallakrabreen", use_cache: bool = True):
    ...
    # plt.subplot(211)
    axes[0].fill_between(lengths["date"], lengths["lower"], lengths["upper"], alpha=0.3)
    axes[0].plot(lengths["date"], lengths["median"])
    axes[0].set_ylabel("Length (m)")

    xlim = plt.gca().get_xlim()
    axes[0].hlines(0, *xlim, colors="black", linestyles=":", alpha=0.5, zorder=0)
    axes[0].set_xlim(xlim)
    # plt.subplot(212)
    axes[1].fill_between(diff_per_day.index, diff_per_day["lower"], diff_per_day["upper"], alpha=0.3)
    axes[1].plot(diff_per_day.index, diff_per_day["median"])
    axes[1].set_ylabel("Advance speed (m/d)")
    # xlim = plt.gca().get_xlim()
    axes[1].hlines(0, *xlim, colors="black", linestyles=":", alpha=0.5, zorder=0)
    # plt.xlim(xlim)
    axes[1].grid()
